				...
				<el-row>
					...
					<el-col :span="12">
						<el-form-item label="客户账号" prop="kehuzhanghao">
							<el-select
								class="list_sel"
								:disabled="!isAdd||disabledForm.kehuzhanghao?true:false"
								v-model="form.kehuzhanghao" 
								placeholder="请选择客户账号"
								@change="kehuzhanghaoChange">
								<el-option v-for="(item,index) in kehuzhanghaoLists" :label="item" :value="item">
								</el-option>
							</el-select>
						</el-form-item>
					</el-col>
...
<script setup>
	...
	const init=(formId=null,formType='add',formNames='',row=null,table=null,statusColumnName=null,tips=null,statusColumnValue=null)=>{
		...
		context?.$http({
			url: `option/kehu/kehuzhanghao`,
			method: 'get'
		}).then(res=>{
			kehuzhanghaoLists.value = res.data.data
		})
		//由上级字段带出不可改
		disabledForm.value.kehuxingming = true;
	}
	...
	const kehuzhanghaoChange=()=>{
		context?.$http({
			url: `follow/kehu/kehuzhanghao?columnValue=` + form.value.kehuzhanghao,
			method: 'get'
		}).then(res=>{
			if(res.data.data.kehuxingming){
				form.value.kehuxingming = res.data.data.kehuxingming
			}
		})
	}
	//富文本
